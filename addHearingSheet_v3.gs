...
function addQuestions() {
  ...
  form.setDescription(
    ...
    '※ 月額のサーバー代は原則かかりません（Cloudflare Pages＋GitHub利用）。独自ドメインを使う場合のみ、年1,500〜4,000円ほどの実費がかかります。'
  );
  ...
  form.addMultipleChoiceItem().setTitle('制作費用のご予算は？')
    .setHelpText('月額のサーバー代は原則0円（独自ドメイン利用時のみ年1,500〜4,000円ほどの実費）。')
  ...
  form.addMultipleChoiceItem().setTitle('ホームページの公開・管理アカウントについて')
    ...
    .setChoiceValues(['自分のGitHub・Cloudflareアカウントで持ちたい（作り方も教えてほしい）', 'まずは制作者におまかせ（あとで自分に移せると嬉しい）', 'よくわからない・相談したい']);
